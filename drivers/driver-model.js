...
function getDriverCriteria(DriverID, callback) {
    sql.query('select * from criteria where driverid = ? ', [DriverID],function(err, results){
        if (err) {
            throw err;
        }
...
module.exports.getDriverCriteria = getDriverCriteria;
